...
public class Program
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        try
        {
            var items = await parser.ParseFromUrlAsync("http://felas87dz.icu:80/get.php?username=729767765&password=551952986&type=m3u_plus");
            const string outputPath = "canais.json";
    ...
    private static async Task GravarNoMongoAsync(string jsonFilePath)
    {
        try
        {
            ...
            var collection = database.GetCollection<M3UItem>("canais");
